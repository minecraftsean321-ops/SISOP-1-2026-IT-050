...
./z_data/penghuni.csv >> ./z_log/tagihan.log
...
1)
	...
	if grep -q "^.*,$kamar" ./z_data/penghuni.csv; then 
	...
	echo "$nama,$kamar,$tanggal,$sewa,$status" >> ./z_data/penghuni.csv
...
2)
	...
	if ! grep -q "$hapusnama" ./z_data/penghuni.csv; then 
	echo "\e[31m[!] Error: Nama penghuni tidak ditemukan \e[0m"

	else 
	
	data_penghuni=$(grep "$hapusnama" ./z_data/penghuni.csv)
	...
	echo "$data_penghuni, $tgl_keluar" > ./z_sampah/history_hapus.csv

	#hapus dari database utama
	grep -v "$hapusnama" ./z_data/penghuni.csv > temp.csv && mv temp.csv ./z_data/penghuni.csv
...
3)
	...
	awk -F "," 'NR>0 {printf "%d | %s          |  %s  | %s       | %s |\n", NR, $1, $2, $4, $5}' ./z_data/penghuni.csv | column -t -s "|" -o " | "
	echo "--------------------------------------------------------------"
	penghuni=$(awk 'END {print NR}' ./z_data/penghuni.csv)
	Aktif=$(awk -F "," ' $5 ~ /Aktif/ {count++} END {print count+0}' ./z_data/penghuni.csv)
	Menunggak=$(awk -F "," '$5 ~ /Menunggak/ {count++} END {print count+0}' ./z_data/penghuni.csv)
...
4)
	...
	if grep -q "$namabaru" ./z_data/penghuni.csv; then
	awk -F "," -v nama=$namabaru '$1 ~ nama {printf "%s     | %s |%s   | %s \n", $1, $2, $4, $5}' ./z_data/penghuni.csv | column -t -s "|" -o " | "
	echo "------------------------------------"
	read -p "Masukkan Status Baru (Aktif/Menunggak): " statusbaru
	if [ "$statusbaru" == "Aktif" ]; then
	sed -i "/$namabaru/s/Menunggak/Aktif/" ./z_data/penghuni.csv
	elif [ "$statusbaru" == "Menunggak" ]; then
	sed -i "/$namabaru/s/Aktif/Menunggak/" ./z_data/penghuni.csv
	else 
...
5)
	...
	echo "=============================================="
        ...
        echo "=============================================="
	if [ ! -s ./z_data/penghuni.csv ]; then
	echo -e "\e[31m[!] Data penghuni kosong!\e[0m"
	else
	pemasukan=$(awk -F "," '/Aktif/ {sum+=$4} END {print sum+0}' ./z_data/penghuni.csv)
	tunggakan=$(awk -F "," '/Menunggak/ {sum+=$4} END {print sum+0}' ./z_data/penghuni.csv)
	jumterisi=$(awk 'END {print NR}' ./z_data/penghuni.csv)
	...
	echo "=============================================="
	tgl_rekap=$(date "+%Y-%m-%d %H:%M")
	echo "[$tgl_rekap] Pemasukan: $pemasukan,Tunggakan: $tunggakan,Terisi: $jumterisi" >> ./z_rekap/laporan_bulanan.txt
	echo -e "\n\e[32m[✓] Laporan berhasil disimpan ke rekap ./zz_rekap/laporan_bulanan.txt\e[0m"
	fi
	echo "Tekan [ENTER] untuk kembali ke menu"
        read # Menunggu user menekan enter
...
2)
	...
	(crontab -l 2>/dev/null; echo "$menit_baru $jam_baru * * *  $(readlink -f $0)/kost_slebew.sh --check-tagihan") | crontab -
